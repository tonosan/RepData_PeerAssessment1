...
author: "Antonio Martínez Pineda"
date: "8 de septiembre de 2015"
output: html_document
---
```{r,echo=FALSE}
knitr::opts_chunk$set(tidy=TRUE,comment=NA,warning=FALSE,message=FALSE) 
```

This is the analysis for the personal activity's dataset of the Reproducible Research Course. Data are in a comma-separated-value file (.csv) and the first action is obtaining the data. For this we use the `download.file` and `unzip` functions to download and unzip the file. After this we load data into R session.
...
nfile<-"activity.zip"
download.file(urldir,destfile = nfile,method = "libcurl")
unzip("activity.zip")
...
stepsByweekday
library(lattice)
xyplot(weekday.mean~interval|wday.wend,data = stepsByweekday,type="l",main="Average number of steps across weekday and weekend",xlab="Interval",ylab="Number of steps",layout=c(1,2))
```
